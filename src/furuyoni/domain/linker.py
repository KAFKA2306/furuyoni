...
class Linker:
    def __init__(self, mapping: Dict[str, str] = TERM_MAPPING):
        self.mapping = mapping
        self.sorted_terms = sorted(mapping.keys(), key=len, reverse=True)
    ...
    def _build_pattern(self):
        ...
        terms_pattern = "(" + "|".join(re.escape(t) for t in self.sorted_terms) + ")"
    ...
    def process_content(self, content: str, file_path: Path) -> str:
        def replace_func(match):
            ...
            if g2:
                term = g2
                target_file_key = self.mapping[term]
                
                rel_source = get_relative_path(file_path, DOCS_DIR)
                if rel_source == target_file_key:
                    return term
                    
                target_abs = DOCS_DIR / target_file_key
                link_url = get_relative_path(target_abs, file_path.parent)
