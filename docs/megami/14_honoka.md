# ホノカ

<div class="megami-header" style="display: flex; align-items: center; gap: 20px; margin-bottom: 20px;">
  <img src="../assets/images/chara/ico-14.png" width="100">
  <div class="megami-meta">
    - **権能**: 旗
    - **難易度**: ★☆☆☆☆
    - **得意間合**: 3-5
  </div>
</div>

## 本質的な解説

ホノカの本質は**「精霊との絆を通じた段階的な成長と守護の完成」**にあります。

彼女は最初から圧倒的な火力を持っているわけではありません。しかし、カードを使い込む（開花させる）ことで、最初は非力だった精霊の力が強力な大精霊へと進化していきます。中盤以降のホノカは、鉄壁の守りと回避困難な連続攻撃を併せ持つ、非の打ち所のない「聖女」となります。

成長の楽しみを最も味わえる、初心者にも扱いやすいながら奥の深いメガミです。

## キーワード能力: 守護 / 開花

あなたが特定のカードを使用、あるいは特定の条件を満たすことで、カードがより強力な形態へと「開花」していきます。

また、対応札が充実しており、自分や味方を「守護」する能力に長けています。最初は小さく芽を出し、最後には大輪の花を咲かせるように、長期的なプランニングで戦場を支配しましょう。

## 通常札の一覧

<div class="grid cards" markdown>

-   [![ホノカ Card](../assets/images/card/cards/na_14_o_n/na_14_o_n_1.png)](../assets/images/card/cards/na_14_o_n/na_14_o_n_1.png){ .glightbox }
    **精霊式**: 基本の攻撃。

-   [![ホノカ Card](../assets/images/card/cards/na_14_o_n/na_14_o_n_2.png)](../assets/images/card/cards/na_14_o_n/na_14_o_n_2.png){ .glightbox }
    **守護霊式**: 守りを固める。

-   [![ホノカ Card](../assets/images/card/cards/na_14_o_n/na_14_o_n_3.png)](../assets/images/card/cards/na_14_o_n/na_14_o_n_3.png){ .glightbox }
    **突撃霊式**: 攻め込む起点。

-   [![ホノカ Card](../assets/images/card/cards/na_14_o_n/na_14_o_n_4.png)](../assets/images/card/cards/na_14_o_n/na_14_o_n_4.png){ .glightbox }
    **桜花のお守り**: 反射で身を守る。

-   [![ホノカ Card](../assets/images/card/cards/na_14_o_n/na_14_o_n_5.png)](../assets/images/card/cards/na_14_o_n/na_14_o_n_5.png){ .glightbox }
    **桜の翅**: 華麗に移動。

-   [![ホノカ Card](../assets/images/card/cards/na_14_o_n/na_14_o_n_6.png)](../assets/images/card/cards/na_14_o_n/na_14_o_n_6.png){ .glightbox }
    **指揮**: 味方を支援。

-   [![ホノカ Card](../assets/images/card/cards/na_14_o_n/na_14_o_n_7.png)](../assets/images/card/cards/na_14_o_n/na_14_o_n_7.png){ .glightbox }
    **追い風**: 行動を加速。

</div>

## 戦術の核心

1. **序盤のセットアップ**: まずは『突撃霊式』や『精霊式』を使い、開花に必要な条件を整えましょう。序盤をどう穏やかに過ごすかがホノカの課題です。
2. **守りの美徳**: 『桜花のお守り』などの対応札で相手のバーストターンをやり過ごします。相手が攻め疲れた時こそ、こちらの開花した精霊が真価を発揮する時です。
3. **『四季はまた巡り来る』の循環**: 倒れても倒れても立ち上がる不屈の精神こそがホノカの最大の武器です。リソースを循環させ、絶え間ない攻勢を維持しましょう。
