# サリヤ

<div class="megami-hud" style="display: grid; grid-template-columns: 100px 1fr; gap: 20px; background: rgba(0,0,0,0.05); padding: 20px; border-radius: 10px; border-left: 5px solid #ec407a;">
  <img src="../assets/images/card/cards/na_11_o_n/na_11_o_n_1.png" width="100">
  <div class="hud-content">
    <div style="font-size: 1.5em; font-weight: bold; margin-bottom: 5px;">【騎】サリヤ / Season 10</div>
    <div style="display: flex; gap: 15px; font-size: 0.9em;">
      <span>難易度: ★★★★☆</span>
      <span>[間合](../rules.md#range): 2-4</span>
      <span>タイプ: 爆発的ビートダウン・機動制御</span>
      <span style="color: #d32f2f; font-weight: bold;">S10 Meta: Tier 1 (Excellent)</span>
    </div>
  </div>
</div>

## S10 環境分析
```mermaid
graph LR
    A[燃料リソースの確保] --> B(超高速移動)
    B --> C{間合破壊}
    C --> D[対応不可の打点]
    C --> E[レンジロックの粉砕]
    D --> F(勝利条件の達成)
    E --> F
```

!!! info "蒸気の咆哮、全戦域を蹂躙する鋼の駿馬"
    シーズン10（大[切札](../rules.md)時代）においても、その圧倒的な機動力による「[間合](../rules.md#range)破壊」性能は随一であり、対戦相手に常に緊張感を与え続けます。

## 物語の起源：サリヤ

> 「加速しろ、私の心臓！ 蒸気の咆哮が、運命の先を切り拓く！」

サリヤは、蒸気の力と強靭な意志で戦場を蹂躙する「騎兵」のメガミです。
彼女の起源は、限界を突破しようとする人類の挑戦心と、鋼の駿馬とともに「どこまでも遠くへ」駆け抜けようとする自由への渇望にあります。彼女にとって、速度こそが力であり、加速に身を委ねる瞬間こそが真の悦びです。
彼女が跨る「ガジェット」は、メガミの力を物理的な運動エネルギーへと昇華させ、あらゆる障害を力強く跳ね飛ばします。
「燃料」という力は、彼女が自らの情熱（リソース）を燃焼させて爆発的な推進力を生み出し、相手の予測する限界速度（[間合](../rules.md#range)）を遥かに超越していく「進歩」の力を象徴しています。

## キーワード能力: 燃料 (Fuel)

*   **燃料の仕組み**: 自分の[オーラ](../rules.md#aura)や[フレア](../rules.md#flare)にある結晶を「燃料タンク」に移動させることで蓄積します。
*   **燃焼の効果**: カード使用時に燃料を消費することで、攻撃力の増加、移動距離の延長、特殊効果の付与などが行われます。
*   **戦略的意味**: 「いつ燃料を貯め、いつ一気に燃やすか」というシビアなリソース管理が、サリヤ使いの醍醐味です。

---

## シーズン10における立ち位置

*   **大[切札](../rules.md)の安定運用**: 圧倒的な機動力で相方の重量級[切札](../rules.md)を当てるための「お膳立て」を一人で完遂。
*   **レンジロック対策**: 遠距離メガミに対しても、燃料を燃やした高速[前進](../rules.md#advance)によって無理やり懐に潜り込みます。

---

## [通常札](../rules.md)解説

### [N1 Burning Steam](cards.md#Burning_Steam)

![Burning Steam](../assets/images/card/cards/na_11_o_n/na_11_o_n_1.png)

### [N2 Waving Edge](cards.md#Waving_Edge)

![Waving Edge](../assets/images/card/cards/na_11_o_n/na_11_o_n_2.png)

### [N3 Turbo Switch](cards.md#Turbo_Switch)

![Turbo Switch](../assets/images/card/cards/na_11_o_n/na_11_o_n_6.png)

### [N4 Steer](cards.md#Steer)

### [N5 Shield](cards.md#Shield)

### [N6 Roar](cards.md#Roar)

![Shield](../assets/images/card/cards/na_11_o_n/na_11_o_n_3.png)
![Steer](../assets/images/card/cards/na_11_o_n/na_11_o_n_4.png)
![Roar](../assets/images/card/cards/na_11_o_n/na_11_o_n_5.png)

### [N7 Omega-Burst](cards.md#Omega-Burst)

![Omega-Burst](../assets/images/card/cards/na_11_o_n/na_11_o_n_7.png)

---

## [切札](../rules.md)解説

### S1 Alpha-Edge

![Alpha-Edge](../assets/images/card/cards/na_11_o_s/na_11_o_s_1.png)

### S2 Beta-Turbine

![Beta-Turbine](../assets/images/card/cards/na_11_o_s/na_11_o_s_2.png)

### S3 Gamma-Ray

![Gamma-Ray](../assets/images/card/cards/na_11_o_s/na_11_o_s_3.png)

---

## 主要アーキタイプ

### 1. 超高速バーストアグロ (Steam Burst)
**「見えた時には、もう死んでいる。」**
燃料によって常勝の1ターンを作り出し、一気に[間合](../rules.md#range)を詰めて相手を沈める。

*   **基本戦術**:
    1. 序盤に『Turbo Switch』で燃料を満タンにする。
    2. 燃料を全消費して『Burning Steam』を連打。
    3. 移動効果を駆使して相手の対応札をスカす。
*   **推奨パートナー**: [ライラ](12_laila.md)(爪)、[ユリナ](01_yurina.md)(刀)

### 2. 万能騎兵コントロール (Flexible Cavalry)
**「変幻自在の盤面支配。」**
形態変化（Transform）を駆使し、相手が得意とする[間合](../rules.md#range)や戦法を封じ込めるスタイル。

*   **基本戦術**:
    1. 相手に合わせて形態を選択。
    2. サイヤの機動力で常に相手の「弱点[間合](../rules.md#range)」に張り付く。
*   **推奨パートナー**: [トコヨ](04_tokoyo.md)(扇)、[サイネ](02_saine.md)(薙)

---

## おすすめの組み合わせ (Pairs)

### [ハガネ](08_hagane.md) (槌騎)
**「鋼鉄の進撃」**
サリヤの移動が[ハガネ](08_hagane.md)の「[遠心](../rules.md#enshin)」条件を無償で達成させる、最高最悪の移動コンボ。全メガミの中でもトップクラスの平均移動距離を誇ります。

### [ライラ](12_laila.md) (拒騎)
**「神速の双璧」**
互いの移動能力が相乗効果を生む超攻撃型ペア。道中で3点削り、『金箱』『りっこ』『果て』を構えつつ『遺響壁』で守りを固め、最後の一撃を叩き込む構成が強力。

### [シンラ](07_shinra.md) (社拒)
**「論理的加速」**
『ひらり』の代わりに『はらり』を採用することで、より強固な盤面を築くことができる。

---

## 戦術の核心

!!! danger "燃料切れ＝ただの少女"
    燃料がないサリヤは、火力の低いメガミに成り下がります。
    **「次のターンのために燃料を残すか、今使い切って勝負を決めるか」**の判断ミスは命取りです。
    特に[オーラ](../rules.md#aura)から燃料を作ると防御がガラ空きになるため、カウンターには細心の注意を払ってください。

!!! tip "「[間合](../rules.md#range)の暴力」を楽しめ"
    サリヤにとって、[間合](../rules.md#range)1から10までの移動は造作もありません。
    相手が「この距離なら安全だ」と[宿し](../rules.md#yadoshi)を行った隙に、燃料1つで5マス[前進](../rules.md#advance)して『Burning Steam』を叩き込む。
    **相手の想定する「ふるよにの常識（移動コスト）」を破壊することこそが、サリヤの真の力です。**
